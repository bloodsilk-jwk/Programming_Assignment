...
bool ColorShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, 
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, float brightnessValue)
{
	bool result;

	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, brightnessValue);
	if(!result)
	{
		return false;
	}
	...
	const WCHAR *fileName)
{
	...
	BrightnessBufferDesc.Usage = D3D11_USAGE_DYNAMIC;				// The buffer is updated each frame.
	BrightnessBufferDesc.ByteWidth = sizeof(BrightnessBufferType);
	BrightnessBufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	BrightnessBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;	// Writing data by CPU
	...
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, float brightnessValue)
{
	...
	BrightnessBufferType* brightnessPtr;
	...
	brightnessPtr = (BrightnessBufferType*)mappedResource.pData;

	brightnessPtr->brightness = brightnessValue;
