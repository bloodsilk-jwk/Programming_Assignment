						  ...
						  float screenDepth, float screenNear)
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		m_worldMatrix[i] = XMMatrixIdentity();
	}
...
bool D3DClass::InitializeRasterizer()
{
	...
	// Create the solid rasterizer state
	result = m_device->CreateRasterizerState(&rasterDesc, &m_SolidCullBack);
	if (FAILED(result))
	{
		return false;
	}
	...
	result = m_device->CreateRasterizerState(&rasterDesc, &m_WireframeCullBack);
	if (FAILED(result))
	{
		return false;
	}
	
	// Create the wireframe rasterizer state (None CullBack).
	rasterDesc.CullMode = D3D11_CULL_NONE;
	result = m_device->CreateRasterizerState(&rasterDesc, &m_Wireframe);
	if (FAILED(result))
	{
		return false;
	}

	// Create the solid rasterizer state (None CullBack).
	rasterDesc.FillMode = D3D11_FILL_SOLID;
	result = m_device->CreateRasterizerState(&rasterDesc, &m_Solid);
	if (FAILED(result))
	{
		return false;
	}
	...
	m_RasterizerState = m_SolidCullBack;
...
void D3DClass::Shutdown()
{
	...
	if (m_SolidCullBack)
	{
		m_SolidCullBack->Release();
		m_SolidCullBack = 0;
	}
	if (m_WireframeCullBack)
	{
		m_WireframeCullBack->Release();
		m_WireframeCullBack = 0;
	}
...
void D3DClass::BeginScene(float red, float green, float blue, float alpha)
{
	...
	{
		if (isCullBack)
			m_RasterizerState = m_WireframeCullBack;
		else
			m_RasterizerState = m_Wireframe;
	}
	else
	{
		if (isCullBack)
			...
			m_RasterizerState = m_Solid;
	}

	// Set the rasterizer state to selected one.
	m_deviceContext->RSSetState(m_RasterizerState);
	return;
}
...
void D3DClass::GetWorldMatrix(XMMATRIX& worldMatrix, int index)
{
	worldMatrix = m_worldMatrix[index];
	return;
}
...
void D3DClass::setSolid()
{
	isWireframe = false;
}

void D3DClass::setWireframe()
{
	isWireframe = true;
}

void D3DClass::setCullBack()
{
	isCullBack = !isCullBack;
}